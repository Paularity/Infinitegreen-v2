...
if (isset($_SESSION["uid"])) {
    ...
    $sql0="SELECT order_id from `orders_info_card`";
    $runquery=mysqli_query($con,$sql0);
    if (mysqli_num_rows($runquery) == 0) {
        echo( mysqli_error($con));
        $order_id=1;
    }else if (mysqli_num_rows($runquery) > 0) {
        $sql2="SELECT MAX(order_id) AS max_val from `orders_info_card`";
	...
	$sql = "INSERT INTO `orders_info_card` 
